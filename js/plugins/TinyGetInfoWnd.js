...
(function() {
  ...
  Window_GetInfo.prototype.initialize = function(id, type, text, value) {
    ...
    // set Y position
    if(yPosType == 0){
      this.y = this.index * 90;
    } else {
      this.y = 520 - (this.index * 90);
    }
    // draw and get item
  ...
  Window_GetInfo.prototype.setup = function(id, type, text, value) {
    ...
    if(type >= 0 && type <= 2) {
      ...
      this.drawItemName(data, 6, 21, 210);
      this.drawText('\xd7', 226, 21, 24, 'center');
      this.drawText(String(Math.abs(value)), 230, 21, 32, 'right');
      this.drawText(this.description(data), 284, 21, 522, 'left');
/*
      this.drawText('\xd7', 306, 17, 24, 'center');
      this.drawText(String(Math.abs(value)), 330, 17, 32, 'right');
      this.drawText(this.description(data), 384, 17, 432, 'left');
*/
    }
    // draw guide string
    this.contents.paintOpacity = 160;
    this.contents.fontSize = 13;
    this.contents.fillRect(0, 0, this.textWidth(text) + 6, 25, '#000000');
    this.contents.paintOpacity = 255;
    this.changeTextColor(this.normalColor());
    this.drawText(text, 6, 0, 510, 'left');
    // gain item
    if(type >= 0 && type <= 2) {
